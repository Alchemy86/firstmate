...
STANDING_RULES=$(printf '%s\n' \
  ...
  '- **Read files directly by default; script only to filter something genuinely too large to read** (a multi-thousand-line registry, a huge JSON blob, aggregating across many run records). `sed -n`, `grep -n`, `head`, or the file-reading tool is the normal way to look at a file - extracting four fields from something enormous earns a script, answering "what is in this field" does not. A script is code, so it is a place to be wrong: one already reported a config had zero goals because it grepped for `goals` instead of the real key `transitional_goals`, and another'\''s `pgrep` pattern matched a sleeping parent instead of the working child and nearly produced a false crash report - both were failures of the tooling written to avoid reading, not of the reading itself. When you do script something, its output is a claim like any other: sanity-check it against a direct read of a small slice before you build on it or report it, because no rows back usually means the query is wrong, not that the thing is absent.')
if [ "$KIND" = ship ]; then
STANDING_RULES=$(printf '%s\n%s\n' "$STANDING_RULES" \
