...
The inventory below is that layout in full: the tracked shared surfaces, the private per-home directories, and the individual records inside them.
It names each record and routes it to its format and lifecycle owner; it does not restate those owners' contracts.
A record marked "never touch" is coordination state whose owner alone may write it.
...
$HOME/.config/fm-telegram.env  optional Telegram captain-comms token and chat id; OUTSIDE this repo, one file per machine, gitignored; presence-gates the `bin/fm-tg-*` scripts and Stop hooks (docs/telegram.md)
$HOME/.config/firstmate/discord.env  optional Discord bot token for OUTBOUND-ONLY fleet updates; OUTSIDE this repo, gitignored; presence-gates the `bin/fm-dc-*` scripts, which nothing calls automatically. Discord never carries captain instructions inbound - Telegram remains the only inbound channel (docs/discord.md)
config/discord-channels.env  generated Discord channel map and measured attachment ceiling; LOCAL, gitignored; written by bin/fm-dc-setup.sh (docs/discord.md)
...
config/pr-watch.env  PR review trigger settings: the forge owner, the repositories this home watches, and the logins that count as ours; LOCAL, gitignored; bin/fm-pr-watch.sh refuses to arm or check without it rather than watching nothing; see docs/configuration.md "PR review triggers"
config/watched-tools.json  optional list of the tools this home depends on, read by the update check armed with bin/fm-tool-update-check.sh; LOCAL, gitignored, firstmate-maintained but human-editable, and NOT inherited by secondmate homes; see docs/configuration.md "Watched tool updates"
config/x-mode.env    generated Relay watcher cadence; LOCAL, gitignored; source before arming watcher when present
config/tg-mode.env   generated Telegram watcher cadence; LOCAL, gitignored; source before arming watcher when present (docs/telegram.md)
...
state/               runtime records and signals; gitignored
  ...
  pr-comments.check.sh pr-new.check.sh pr-ours.check.sh  generated PR review trigger poll shims and their .check-trust bindings; present only after bin/fm-pr-watch.sh arm; their .pr-<mode>-seen lists and .pr-watch-<mode> records are what keep one PR or comment from being reported on every poll
...
The `state/` inventory above routes the branch's runtime files to their format and lifecycle owners.
