...
A revoked token (401) and a sustained rate limit (429) come back as an error body that `curl` transfers perfectly, so without this they looked exactly like a captain who had not written: no output, no wake, no trace.
...
A getUpdates conflict (409) with the Stop hook's own long poll is deliberately left out of this reporting - see "The watcher's short poll woke firstmate on the waiter's own 409s" below.
...
- **The waiter also spun when Telegram *answered*.** The backoff above only covered a failed transfer, and `curl` reports an HTTP error body as a completely successful one.
  ...
  **Known, accepted limitation (a no-mistakes review finding, 2026-08-23):** the backoff does not distinguish a `409` from any other unusable reply, even though a `409` is the one condition this feature genuinely produces by design (see the poller/waiter race above), not a real problem.
  A busy session racing the watcher's own 30s poll can climb the waiter's backoff toward its 60s cap during entirely normal operation, meaning the *next* genuinely new message can wait up to that long for the waiter's own long poll to try again - though the watcher's independent 30s poll cycle still records and, on Claude, eventually surfaces it regardless, bounding the practical delay well under the waiter's own worst case.
  Exempting `409` specifically would need this script to also capture and inspect `bin/fm-tg-fetch.py`'s stderr reason, which it currently does not (only the exit code); that is a real, scoped fix, just not one to make casually to carefully-tuned retry logic already reshaped by several captain-observed production incidents this session, without the live traffic needed to verify it does not reopen the CPU-spin failure the backoff exists to prevent.
...
- **The watcher's short poll woke firstmate on the waiter's own 409s (2026-08-27).** `bin/fm-tg-poll.sh`'s refusal-reporting path treated a `409` exactly like a `401` or `429`: a standing failure, worth a wake.
  But a `409` here is not a channel problem - it is the single-flight waiter above doing exactly what it is supposed to do, and it recurred every 30s check cycle for as long as the waiter's long poll ran (up to `FM_TG_HOOK_MAX`, default 1800s), each recurrence distinct enough (no countdown to dedup against, unlike the `429` body) to defeat `state/.tg-poll-error`'s own dedup and wake firstmate on a turn with no news, observed about a dozen times in one session.
  `bin/fm-tg-poll.sh` now reads `state/.tg-hook.lock`'s own recorded pid and asks `fm_pid_alive` (`bin/fm-wake-lib.sh`) whether it is a live process before ever calling `curl`; while it is, the poll does nothing and says nothing.
  Checking the lock path's existence instead would have been wrong: a claim left behind by a waiter that died without releasing it must never silence this poll, so only a genuinely alive pid defers - a stale claim is polled through exactly as if it were not there.
  As a narrower backstop for a `409` that lands anyway (a waiter that claims the lock just after this check ran, or any timing not yet foreseen), the poll also matches the leading `409` on `bin/fm-tg-fetch.py`'s own refusal reason and treats it as a pure no-op - not printed, not recorded - rather than a reported refusal.
  This is scoped to that one poll and that one code: it does not touch `bin/fm-tg-wait.sh`'s own backoff, so the "Known, accepted limitation" on the waiter's `409` handling two bullets up still stands.
